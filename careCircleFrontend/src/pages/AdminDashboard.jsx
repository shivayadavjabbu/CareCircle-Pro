...
import { getAdminStatistics, getAdminProfile } from "../api/adminApi";
    ...
    const fetchData = async () => {
      try {
        const [statsData, profileData] = await Promise.all([
          getAdminStatistics(),
          getAdminProfile().catch(() => null) // Handle if profile fetch fails (e.g. 404 first time)
        ]);
        setStats(statsData);
    ...
    <div className="min-h-screen pt-[80px] pb-12 px-6 font-sans">
      <div className="max-w-[1024px] mx-auto">
        ...
        <div className="mb-10 flex justify-between items-end">
          <div>
            <h1 className="text-[40px] font-semibold text-[#1d1d1f] tracking-tight">Hi, {adminName}</h1>
            <p className="text-[#86868b] text-[17px]">Admin Dashboard</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-[180px]">
          ...
          {/* Edit Profile Card */}
          <div className="card-apple md:col-span-2 flex flex-col justify-center items-center text-center cursor-pointer group hover:bg-[#f5f5f7]" onClick={() => navigate("/admin-profile")}>
